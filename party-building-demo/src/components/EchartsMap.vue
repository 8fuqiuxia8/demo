	...
	const options = {
		geo: [
			...
			},
			{
				...
				//如果宽高比大于1，则宽度为100，小于1则高度100，保证了不超过100*100的区域
				layoutSize: '70%',
				itemStyle: {
					//图形描边颜色
					borderColor: 'rgba(25,50,86,0.5)',
		...
		//lines路径图 scatter散点图 effectScatter带有涟漪效果的散点图
		series: [
			{
				type: 'lines',
				lineStyle: {
...
const getBarCoordData = () => {
	...
	const result = []
	//遍历方法 数组遍历for forEach map(map有数据返回，forEach无参数返回)
	for(const [name,coords] of dataMap ){
		//console.log(name,coords)
		const coordsNumber = coords.map(parseFloat)
		//...是VUE中的一种格式，意思是拷贝
		result.push({
			coords:[[...coordsNumber],[coordsNumber[0],coordsNumber[1]+0.009]],//[起点经纬度,，终点经纬度]
